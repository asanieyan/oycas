class MemoryProfiler 
  ...
  def self.start(opt={}) opt = DEFAULTS.dup.merge(opt)
    run = 1
    FileUtils.mkdir_p('log/m_profiler')
Thread.new do
  prev = Hash.new(0)
  curr = Hash.new(0)
  curr_strings = []
  ...
  file = File.open('log/m_profiler/memory_profiler.log','w')
  ...
  loop do
    begin
      ...
      if opt[:string_debug]
        File.open("log/m_profiler/memory_profiler_strings.log.#{Time.now.to_i}",'w') do |f|
